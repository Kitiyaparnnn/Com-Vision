...
#try mode cv2.RETR_EXTERNAL(only outside objects)and cv2.RETR_TREE(thoroughly objects)
#try method cv2.CHAIN_APPROX_NONE and cv2.CHAIN_APPROX_SIMPLE

#print('Hierarchy:\n', 'Left Right Child Parent\n', hierarchy) 
#if Parent == -1 means its parent is Root.

print('Types of contours:', type(contours)) 
print('Length of contours:', len(contours)) #count every diff colors

n = 0
print('Types of each contour:', type(contours[n]))
print('Shape of each contour:', contours[n].shape) #(points,position)
